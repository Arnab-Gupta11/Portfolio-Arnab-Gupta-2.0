...
const BlogCard = ({ blog }) => {
  return (
    <Link to={`/blogs/${blog._id}`}>
      <div className="w-full bg-card-gradient-light dark:bg-card-gradient rounded-xl  shadow-project-card-shadow-light dark:shadow-project-card-shadow overflow-hidden group  hover:shadow-lg hover:cursor-pointer">
        ...
        <div className="p-6">
          ...
          <p className="text-[#7d3cff] dark:text-[#9864ff] mt-2">{blog?.category}</p>
        </div>
      </div>
    </Link>
  );
